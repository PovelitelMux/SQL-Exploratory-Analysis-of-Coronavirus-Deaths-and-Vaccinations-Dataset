...
SELECT 	continent
		...
		, (SUM(new_deaths)::real/SUM(new_cases))*1000 AS death_per_1000_cases
		, (SUM(new_deaths)::real/SUM(population))*1000000 AS death_per_1_million_population
FROM deaths
WHERE continent IS NOT null
GROUP BY continent
ORDER BY 1 ASC;
...
-- clean data in table if needed
TRUNCATE TABLE public.COVID_summary;
...
(
	...
	logarithm_of_new_cases numeric,
	logarithm_of_new_deaths numeric,
	logarithm_of_new_vacc numeric
);
...
SELECT 	dea.continent
		...
		, CASE
			...
			THEN 0
			ELSE LOG(new_deaths) 
			END AS logarithm_of_new_deaths
		, CASE 
			WHEN new_vaccinations = 0
			THEN 0
			WHEN new_vaccinations IS null
			THEN 0
			ELSE LOG(new_vaccinations)
			END AS logarithm_of_new_vacc
FROM deaths AS dea
JOIN vaccinations AS vac
	ON dea.location = vac.location
	AND dea.date = vac.date;
...
SELECT	date
		...
		, CASE
			...
			THEN 0
			WHEN SUM(new_cases) IS null
			THEN 0
			ELSE LOG(SUM(new_cases))
			END AS log_cases
		, CASE
			WHEN SUM(new_deaths) = 0
			THEN 0
			WHEN SUM(new_deaths) IS null
			THEN 0
			ELSE LOG(SUM(new_deaths))
			END AS log_deaths
		, CASE
			WHEN SUM(new_vaccinations) = 0
			THEN 0
			WHEN SUM(new_vaccinations) IS null
			THEN 0
			ELSE LOG(SUM(new_vaccinations))
			END AS log_vaccinations
FROM COVID_summary
WHERE continent LIKE 'Europe'
GROUP BY date
ORDER BY 1;
